-- Create fake users and ratings for testing revenue sharing
-- This creates both users and their ratings with correct schema

DO $$
DECLARE
    target_property_id UUID := 'bd9272e6-c1d4-4c53-8846-20e9a41b2013'; -- 1312 Centennial Court
    ...
    user_emails TEXT[] := ARRAY['alice@test.com', 'bob@test.com', 'carol@test.com', 'david@test.com', 'eve@test.com', 'frank@test.com', 'grace@test.com', 'henry@test.com', 'ivy@test.com', 'jack@test.com', 'kate@test.com', 'liam@test.com', 'mia@test.com', 'noah@test.com', 'olivia@test.com'];
...
BEGIN
    RAISE NOTICE '🎭 Creating fake users and ratings for revenue sharing test...';
    RAISE NOTICE 'Property: 1312 Centennial Court, San Jose, CA';
    RAISE NOTICE 'Property ID: %', target_property_id;
    
    -- First, create the fake users in app_user table (using only columns that exist)
    RAISE NOTICE '';
    RAISE NOTICE '👥 Creating fake users...';
    
    FOR i IN 1..15 LOOP
        current_user_id := fake_users[i];
        user_name := user_names[i];
        user_email := user_emails[i];
        
        -- Insert user if they don't already exist (using only id, email, created_at, updated_at)
        ...
        RAISE NOTICE '👤 Created user: % (%) - %', user_name, user_email, current_user_id;
    END LOOP;
    
    -- Now create ratings for each fake user
    RAISE NOTICE '';
    RAISE NOTICE '⭐ Creating ratings...';
    ...
    FOR i IN 1..15 LOOP
        ...
        RAISE NOTICE '👤 Creating % ratings for %', rating_count, user_name;
    ...
    FOR i IN 1..15 LOOP
        ...
        FROM rating 
        WHERE property_id = target_property_id AND user_id = current_user_id;
    ...
    BEGIN
        ...
        FROM get_top_contributor(target_property_id);
