...
-- This temporarily disables the immutable rating trigger
...
BEGIN
    ...
    -- Temporarily disable the immutable rating trigger
    RAISE NOTICE '🔧 Temporarily disabling rating protection trigger...';
    ALTER TABLE rating DISABLE TRIGGER prevent_rating_update_delete;
    
    -- Clear existing ratings for this property
    DELETE FROM rating WHERE property_id = target_property_id;
    RAISE NOTICE '🧹 Cleared existing ratings for this property';
    ...
    FOR i IN 1..15 LOOP
        ...
        RAISE NOTICE '👤 Creating % ratings for % (%)', rating_count, user_name, current_user_id;
    ...
    -- Re-enable the immutable rating trigger
    RAISE NOTICE '🔧 Re-enabling rating protection trigger...';
    ALTER TABLE rating ENABLE TRIGGER prevent_rating_update_delete;
...
EXCEPTION
    WHEN OTHERS THEN
        -- Make sure to re-enable the trigger even if something goes wrong
        RAISE NOTICE '❌ Error occurred, re-enabling trigger...';
        ALTER TABLE rating ENABLE TRIGGER prevent_rating_update_delete;
        RAISE;
